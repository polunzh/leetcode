...
var maxSubArray = function (nums) {
    var maxStep = nums.length,
        max = null,
        arr;
    if (nums.length <= 1) return nums[0];
    for (var i = 0; i < nums.length; i++) {
        for (var j = 1; j < (maxStep - i + 1); j++) {
            var t = nums.slice(i, (i + j));
            var m = t.reduce((a, b) => a + b, 0);
...
/**
 * 动态规划(DP)问题: https://zh.wikipedia.org/wiki/%E5%8A%A8%E6%80%81%E8%A7%84%E5%88%92
 * 原解: https://discuss.leetcode.com/topic/6413/dp-solution-some-thoughts
 * @param {number} nums
 * @returns {number}
 */
var maxSubArray = function (nums) {
    var n = nums.length;
    var dp = new Array(n);//dp[i] means the maximum subarray ending with A[i];
    dp[0] = nums[0];
    var max = dp[0];
